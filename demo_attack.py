...
def run_demo():
    ...
    open(LOG_PATH, 'w').close() # On vide le log
    write_log({"command": "start_demo"}) # Le Backend bloque les vrais logs
    write_log({"command": "clear", "is_demo": True}) # Le Dashboard se vide
    ...
    open(LOG_PATH, 'w').close() # Vide le fichier
    
    # Inject Green Baseline (Still tagged as Demo so it shows up)
    ts = get_ts()
    ...
    write_log({"command": "stop_demo"}) # Le Backend écoute de nouveau le vrai système !
...
if __name__ == "__main__":
    try:
        ...
        write_log({"command": "stop_demo"}) # Sécurité en cas de Ctrl+C
